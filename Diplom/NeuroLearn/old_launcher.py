...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        self.leDownBorder.setGeometry(90 + self.leDownBorder.width(), 0,
                                      40, 20)
        ...
        # Results
        self.btnResults = QPushButton("Show results", self)
        self.btnResults.clicked.connect(partial(utils.show_results))
        self.btnResults.move(self.btnSignal.x(), self.btnSignal.y() + self.btnResults.height())

        # Compile signal
        self.btnCompileSignal = QPushButton("Compile signal", self)
        self.btnCompileSignal.clicked.connect(partial(utils.compile_signal))
        self.btnResults.move(self.btnSignal.x(), self.btnSignal.y() + self.btnResults.height())
    ...
    def neural_network(self, mode):
        while self.can_calculating:
            thread = threading.Thread(target=NN.main,
                                      args=(mode, self.checkFilter.isChecked(), self.leDownBorder.text(),))
            thread.start()
            thread.join()
            time.sleep(1)
